    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
        ...
        <div class="rounded-2xl p-4 bg-accent/50 border border-blue-300">
            <div class="flex flex-row items-center text-center">
                <h2
                    class="text-blue-300 font-bold flex items-center gap-2 text-2xl"
                >
                    Squadra Blu
                    {#if bluWinners}
                        <span
            ...
            <ul class="divide-y divide-blue-200">
                {#each getTeamPlayers(match.player_match, "blu") as player}
                    <li class="py-2 flex items-center justify-between">
                        <span
                            class="font-medium text-blue-300 flex items-center gap-1.5"
                        >
                            {player.players.name}
                        </span>
        ...
        <div class="rounded-2xl p-4 bg-accent/50 border border-red-400">
            <div class="flex flex-row items-center text-center">
                <h2
                    class="text-red-400 font-bold flex items-center gap-2 text-2xl"
                >
                    Squadra Rossi
                </h2>
                {#if rossiWinners}
            ...
            <ul class="divide-y divide-red-200">
                {#each getTeamPlayers(match.player_match, "rossi") as player}
                    <li class="py-2 flex items-center justify-between">
                        <span
                            class="font-medium text-red-400 flex items-center gap-1.5"
                        >
                            {player.players.name}
                        </span>
                        <span class="flex items-center gap-2">
